...
#pragma omp parallel for private(i)
	...
	// Move doublets and sources downstream, and apply source decay model.
//FIXME: Current model cuts source strength in half with each step downstream,
//       but it would be better to base it on some quantity that doesn't depend
//       on wake discretization.
...
#pragma omp parallel for private(i,j)
	for ( i = 0; int(i) < _nspan-1; i++ )
	{
		...
		if (viscous)
		{
			_quads[i].setSourceStrength(
		           0.5*_tris[i*(_nstream-1)*2+(_nstream-2)*2].sourceStrength());
			for ( j = _nstream-2; j >= 1; j-- )
			{
				_tris[i*(_nstream-1)*2+j*2].setSourceStrength(
				           0.5*_tris[i*(_nstream-1)*2+(j-1)*2].sourceStrength());
				_tris[i*(_nstream-1)*2+j*2+1].setDoubletStrength(
				         0.5*_tris[i*(_nstream-1)*2+(j-1)*2+1].sourceStrength());
			}
		}
	}
}
